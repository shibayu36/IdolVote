package IdolVote::Context;

use strict;
use warnings;
use utf8;

use IdolVote::Request;
use IdolVote::Config;
...
sub _build_request {
    ...
    return IdolVote::Request->new($self->env);
};
...
sub _build_route {
    my $self = shift;
    return IdolVote::Config->router->match($self->env);
};
...
sub render_file {
    ...
    require IdolVote::View::Xslate;
    my $content = IdolVote::View::Xslate->render_file($file, {
        c => $self,
        %{ $self->stash },
        %$args
    });
...
sub error {
    ...
    IdolVote::Error->throw($code, $message, %opts);
}
...
sub _build_db {
    my ($self) = @_;
    return IdolVote::DBI::Factory->new;
}
