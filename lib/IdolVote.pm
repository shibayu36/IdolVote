...
use IdolVote::Error;
use IdolVote::Context;
use IdolVote::Config;
use IdolVote::Logger qw(critf);
...
sub run {
    ...
    my $context = IdolVote::Context->from_env($env);
    my $dispatch;
    try {
        my $route = $context->route or IdolVote::Error->throw(404);
        $route->{engine} or IdolVote::Error->throw(404);
        $env->{'idolvote.route'} = $route;
        ...
        my $handler = $engine->can($action) or IdolVote::Error->throw(501);
